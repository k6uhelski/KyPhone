...
| upload | ADD FROM A COMPUTER: the address to open on a computer on the same Wi-Fi and a four-digit code; what arrives is listed (last three). **The page exists only while this screen is open**; Enter / Esc / Q stop it and return to settings. Not on a network → a stop alert |
