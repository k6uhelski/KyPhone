...
| Note | `NOTE\|hdr\|line·line·…` — hdr `''` typing (the cursor follows the last line) / `B` `<` selected / `D` DELETE selected; the Radxa wraps the note at 30 columns (a paragraph break is an empty line) and sends the **end** that fits, first line `...` when the start is cut |
