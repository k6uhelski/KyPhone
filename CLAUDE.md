...
| netlist | a scan starts on opening (SCANNING... until it answers; only Esc/Q works meanwhile). RESCAN leads, then networks (Wi-Fi: Secured/Open, CONNECTED) or devices (Paired/New device). An empty scan says `No networks found` / `No devices found` under RESCAN. Enter: RESCAN scans again; an open network connects at once; a secured one opens the password box; a device pairs (if needed) and connects. Esc → settings |
| netpass | typing fills the password (only `*` go to the screen; Q is a letter here). ↑ selects the header's `<` (the phone keyboard has no Esc), ↓ back to the field, Enter on `<` → the network list without connecting; typing while `<` is selected returns to the field. Enter in the field connects |
| netstate | WORKING (Q cancels; the late result is dropped) → OK (Enter → settings) or FAIL, which has a COULDN'T CONNECT heading (Enter → the password box with the typing kept, or the list) |
