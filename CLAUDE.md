...
| 0.6.4 | the **screen light times out** (on with a key, off after 5 s idle; `LIGHT\|n`), **full refresh only on the lock screen and when a feature opens** (partial inside a feature; frame start byte 0x04), and the home status bar is never selected | branch `settings-and-modem`; flashed and deployed 2026-09-26 |
| 0.6.5 | the Wi-Fi page laid out like a phone's: it opens on the joined network, and the rest sit under a small **OTHER NETWORKS** label (a section row: `#`-prefixed title, 56 px, never selected) with SEARCHING... beside it | branch `settings-and-modem`; flashed and deployed 2026-09-26 |
| 0.6.6 | a **SAVE** button in the note editor's header (`< SAVE DELETE`; Up lands on SAVE) | branch `settings-and-modem`; flashed and deployed 2026-09-26 |
| 0.6.7 | **no flashing while scrolling**: the Inkplate library's hidden every-10th-update full refresh is switched off (`setFullUpdateThreshold(0)`); a full refresh happens only moving between the home screen and a feature (either way) and going to the lock screen | branch `settings-and-modem`; flashed and deployed 2026-09-26 |
| **0.6.8** | now-playing's time moves every **5 s** (was 30); the first screen after the light wakes no longer waits 3 s (a LIGHT frame no longer waits for the busy signal it is too quick to give) | branch `settings-and-modem`; flashed and deployed 2026-09-26 |
...
| :--- | :--- |
| lock | any → home |
| home | ↑↓ move; the status bar at the top is never selected (Kyle, 2026-09-26: ↑ stops at TEXT); Q/Esc → lock. Enter opens TEXT / CALL / READ / LISTEN / CONTACTS / NOTES / SETTINGS (list windows reset). The row after LISTEN's icon shows a three-bar mark while music plays. Esc → lock. `i` = incoming-call demo. Menu order (Kyle's, 2026-09-19: texts, calls, books, music, address book; NOTES and SETTINGS added after): TEXT, CALL, READ, LISTEN, CONTACTS, NOTES, SETTINGS — the design handoff had CONTACTS third; only the first three rows are on screen without scrolling. The order lives in four places that must agree (`kyphone_os.HOME_MENU`, `simulator.HOME_MENU`, `labels[]` in `ui_screens.h`, `tools/make_icons.py` ORDER → the generated icon tables); a test checks them |
...
| contact | top row `[back, edit]`, action row `[call, text]` (saved) · `[add number]` (saved, no number) · `[call, text, create contact]` (not in contacts, no EDIT; CREATE CONTACT is too wide for the row, so it draws on a second line, while ← → still step through the three in order). CREATE CONTACT opens the form with the number filled in |
...
| reader | → ↓ Enter Space = next page; ← ↑ Backspace = previous; `+`/`=` bigger text, `-`/`_` smaller; **`c` → CHAPTERS**; Esc/`q` → library. The ends of the book and of the size range raise a stop alert; Enter/Esc returns to the page |
| chapters | the book's own table of contents (from the EPUB's nav/NCX; nothing is parsed to build it), each with how far into the book it starts; the chapter being read is marked HERE and selected. ↑↓ (windowed, 5 rows); Enter jumps to the start of that chapter (full refresh); Esc/Q or Enter on the header → back to the same page. A book with no table of contents → a stop alert |
...
| settings | five rows: Wi-Fi and Bluetooth (each with its status; Enter opens that list), **Screen light** (Level n of 8 / Off; Enter opens SCREEN LIGHT), **Activity mark** (ON/OFF; Enter switches it) and **Add from a computer** (Enter opens it). ↑↓; ↑ past the first → header (Enter → home); Esc → home. Saved in `data/settings.json` |
| upload | ADD FROM A COMPUTER: the address to open on a computer on the same Wi-Fi and a six-digit code; what arrives is listed (last three). **The page exists only while this screen is open**; Enter / Esc / Q stop it and return to settings. Not on a network → a stop alert |
| light | SCREEN LIGHT: ← ↓ dimmer, → ↑ brighter, 0 (off) to 8, saved as it changes (a step past either end redraws); Enter/Esc → settings. The Inkplate sets its front light from the same command |
| wifi | the Wi-Fi switch (Enter turns it on/off); the joined network, **selected when the page opens** (Enter: FORGET THIS NETWORK, on the confirmation screen); then a small **OTHER NETWORKS** label (never selected; the arrows skip it) with `SEARCHING...` or `NONE FOUND` beside it, and the networks nearby, A–Z. A search runs once on opening (one redraw when it answers); `SEARCH AGAIN` at the end runs another. An open network connects at once; a secured one opens the password box. Esc/Q → settings |
| bluetooth | the Bluetooth switch; the paired devices, A–Z, Connected / Not connected (Enter: FORGET or RE-CONNECT, RE-CONNECT the default); `PAIR NEW DEVICE`. **Nothing scans here.** Bluetooth is not switched off while the keyboard is connected by it, and a keyboard (any input device) is never forgotten, connected or not (Kyle, 2026-09-25): a stop alert says why. Esc/Q → settings |
| btpair | OTHER DEVICES: one search for devices not yet paired (`SEARCHING...`, then the list and `SEARCH AGAIN`, `No devices found` when empty). Enter pairs and connects. Esc/Q → bluetooth |
| netpass | typing fills the password (only `*` go to the screen; Q is a letter here). ↑ selects the header's `<` (the phone keyboard has no Esc), ↓ back to the field, Enter on `<` → the Wi-Fi list without connecting; typing while `<` is selected returns to the field. Enter in the field connects |
| netstate | WORKING (Q cancels; the late result is dropped) → OK (Enter → the refreshed Wi-Fi / Bluetooth list) or FAIL, which has a COULDN'T CONNECT heading (Enter → the password box with the typing kept, or the list it came from). Forgetting a network or device also shows WORKING, then returns to its list |
| notes_list | the notes, newest first: the first line of each (≤22 characters, `...` when cut) over when it was last changed. ↑↓; ↑ past the first → header (←→ back/plus); Enter opens a note; `+` or the header + → a new note; Esc → home. **An empty list opens with `+` selected** |
| note | typing adds to the end (Q and WASD are letters here); **Enter starts a new line**; Backspace deletes. ↑ → the header `< SAVE DELETE` (↑ lands on **SAVE**): ←→ between them, ↓ back to the text; Enter on SAVE or `<` (or Esc) saves and returns to the list; Enter on DELETE → confirm (KEEP NOTE is the default). A note saves itself on leaving, moves to the top when changed, and is not kept if empty. ≤4,000 characters (a key past that is not taken). Saved in `data/notes.json` (gitignored) |
| stub | Enter/Esc → the screen it came from, state intact |
| calls_list / dial / outgoing / incoming / in_call | **simulated** — there is no telephony until the cellular modem exists. **Calls are logged when they end** (`data/calls.json`, newest 50, newest first): an answered outgoing call is OUT with its length (m:ss), one cancelled before it connects is OUT with none, an answered incoming call is IN, an incoming call nobody answers is MISS. Each call is logged with the other party's number when it is a dialable one (`number`). **Enter on a log row opens that person's page** (the contact page, CALL selected, so Enter-Enter calls back; TEXT opens the conversation); an older row with no number still redials by name. A conversation shows the newest call with that number as a quiet line under the header (`LAST CALL: MISSED, YESTERDAY`); calls are never matched by display name. The time column reads like messages (clock time, Yesterday, a weekday, a date) |
...
*   **Receiving:** `modem_sms_loop()` polls every `SMS_POLL_INTERVAL` seconds. `poll_new()` lists unread texts in the modem's **UCS2** character set, so every sender and body is one hex line: a text with line breaks, or one that is just "OK", can never be taken for the modem's own reply, and accents survive (then `sanitize()` at display). Read texts are cleared with one `AT+CMGD=1,1` sweep (so a failed clean-up is caught next time and the storage never fills). A text joins the existing conversation with that number however the network wrote it (`resolve_peer`), and no error ever ends the loop.
*   **Sending:** `send()` switches to **IRA** (plain ASCII, which the modem converts to the GSM alphabet itself, so @ $ _ arrive as typed); a text longer than 160 characters goes as several texts split between words (`split_text`; text mode cannot join them). A refusal or a missing `>` prompt cancels text entry (ESC) so the next exchange is not confused.
*   **One exchange at a time:** a lock in `SerialModem` serializes the send worker, the poll loop and start-up, which all share the one port; any serial failure (an unplugged dongle) is raised as `ModemError`. These fixes came from a code review (2026-09-25) and are tested against a scripted fake SIM7600 (`tests/test_modem.py`).
*   **The first real text:** `planning/modem-first-text.md` is the step-by-step, and `tools/modem_check.py` (run on the Radxa) checks the modem, SIM, signal, registration and SMS centre in plain words, then can send a test text and wait for the reply. **pyserial is not installed on the Radxa yet** (`sudo apt install python3-serial`; without it the phone runs with no modem); ModemManager is not installed, so nothing else grabs the port.
...
*   **Refresh policy:** the Radxa asks for a *full* (flashing) refresh when a book opens, on a new chapter, a new font size, after a stop alert and, since 2026-09-20 (Kyle's call, no ghosting), **on every page turn** (`READER_FULL_EVERY = 0`; set it to N to allow N partial turns between full ones).
*   **Text limits:** printable ASCII only (accents stripped, typographic punctuation reduced, anything else `?`); no images, bold or italic. **Chapter menu:** `c` in a book (`Book.contents()`).
...
*   **`network_control.py`** (importable without hardware) wraps `nmcli` and `bluetoothctl`, list-form subprocess calls only, never a shell string. Every call returns a plain result and never raises: a missing tool or a timeout reads as Wi-Fi/Bluetooth not being there, which is what the emulator shows on a Mac.
*   **The flow** (Kyle's choice, 2026-09-25, after trying the first version on the phone: he did not want a scan every time a list opened). Wi-Fi: switch, joined network (Forget This Network), nearby A–Z, searched once on opening. Bluetooth: switch, known devices (Forget / Re-connect), PAIR NEW DEVICE → Other devices; Bluetooth never scans until asked. Forget and Re-connect use the CONFIRM screen, so the firmware only gained the OTHER DEVICES title.
*   **The keyboard guard:** the phone refuses to switch Bluetooth off while an input device is connected by it, and never forgets an input device at all (`bluetoothctl info` shows `Icon: input-…`), since the Bluetooth keyboard is the only way to type and pairing it again would need a keyboard. The Wi-Fi switch has no such guard; switched off, the Radxa also drops off the network (SSH).
*   Scans, connects and pairs run on a background thread; the screen shows WORKING and the keyboard is never blocked. A result that arrives after the phone has moved on is dropped.
*   The Wi-Fi password is held on the Radxa only; the wire carries a `*` mask of the same length (tests check that no typed letter ever reaches a command).
*   **Screen light and the activity mark** (2026-09-25): the front light is the one piece of state the Inkplate holds between frames — hardware, not data — and the Radxa re-sends it in every `LOCK`. It is carried inside drawing commands (`LOCK`, `LIGHTSET`), never as a command of its own, because the sender keeps only the latest command and a separate one could be dropped. A missed call is logged with `seen: false` until the call list is opened; the lock screen redraws when a text arrives while it is showing.
*   **Firmware:** `ui_settings`, `ui_netlist`, `ui_netpass`, `ui_netstate`, `ui_lightset` in `ui_screens.h` (`apply_light()` and the lock mark are in the `.ino`), built and compared with the emulator on the computer (2026-09-24). **Not yet on the phone** — flash the firmware and deploy the Python together (an old firmware draws these commands as stray text). **Not yet proven:** that `nmcli` and `bluetoothctl` are installed on the Radxa and behave as the tests' fake output says; the service runs as root, so no permission prompts are expected.

### **Add from a computer (books, music, contacts)**
*   **`upload_server.py`** (standard library `http.server`, importable without hardware): one small web page, served on port 8080 **only while Settings → Add from a computer is on screen** — opening it starts the server with a fresh six-digit code (`secrets`), leaving it stops it. Uploads are `PUT /upload?kind=books|music|contacts&name=…[&folder=…]` with the code in `X-Code`; the page's JavaScript sends each file raw, so there is no multipart parsing. **The page** (2026-09-26): the code in four boxes (kept for the browser session), one drop area that takes files or whole folders and sorts them by type (a dropped folder's name becomes the album), a progress bar and ✓/✕ per file, and a summary; a wrong code stops the queue and asks again. Checked in headless Chrome against the real server.
*   **Safeguards:** every upload needs the six-digit code, and **10 wrong codes stop the server** (the phone shows a stop alert); **10 minutes with no upload stop it too**; only requests whose Host is the phone's own `ip:port` are answered (blocks DNS rebinding from a web page on the laptop); every reply carries `X-Frame-Options: DENY`, `nosniff`, `no-referrer` and a strict Content-Security-Policy; it is plain HTTP, protected in transit by the Wi-Fi's own encryption (HTTPS on a home device would mean a certificate warning in every browser); only EPUB for books and MP3/M4A/AAC/FLAC/Ogg/Opus/WAV for music; a file name is reduced to plain characters and can never leave its folder (no paths, no leading dots; an album folder is cleaned the same way); nothing is overwritten (`a (2).epub`); size limits (100 MB a book, 500 MB a track, 5 MB contacts) and a free-space check leaving 200 MB; a file is written under a temporary name and renamed only once complete, so a cut-off upload leaves nothing. The page never reads anything back from the phone.
*   **Contacts:** a `.vcf` (Google Contacts, iCloud and phones export one) is parsed (`parse_vcards`: N or FN, the first TEL, folded lines, grouped properties) and merged under the contact form's rules: a first name and a dialable number are needed, and a number already saved is the same person and is skipped; the address book stays alphabetical. The summary (`Contacts: 3 added, 1 already here, …`) shows on the phone and the page.
*   **After an upload:** READ rescans the books folder each time it opens, and the music list picks new files up the same way; nothing needs a restart.
*   **Checked 2026-09-25:** the Radxa's port 8080 is reachable from another computer on the home Wi-Fi (a throwaway server, stopped afterwards). **Not yet proven:** the real page on the phone and uploads of real albums; the server itself is tested end to end on the Mac against 127.0.0.1.
...
*   **Background playback:** leaving LISTEN does not stop the music. `HOME2` carries a sixth field (`1` = playing) for the mark. A ticker redraws now-playing every 5 s while it is showing and playing (a partial refresh; 30 s until 0.6.8, which looked stuck); nothing else is redrawn by the player, except the home menu when the album finishes (so the mark goes out). A bad track only raises its alert if now-playing is on screen; in the background it is skipped silently.
...
| Home | `HOME2\|time\|index\|unread\|style\|playing` — index 0 TEXT 1 CALL 2 READ 3 LISTEN 4 CONTACTS 5 NOTES 6 SETTINGS; `playing` 1 = music is playing (the equalizer mark by the LISTEN row); style `I` icons (default) / `B` icons and words / `W` words |
...
| Thread | `THREAD2\|name\|draft\|hdr\|code·time·text\|…` — hdr `''`/`B`/`I`; code `R` received, `Y0` sending, `Y1` sent, `Y2` not sent, `Y3` not sent and selected, `C` the LAST CALL line (text only; drawn under the header, never dropped for space); ≤3 bubbles |
...
| Network list | `NETLIST\|W/B/P\|sel\|name·sub·right\|…` — W the Wi-Fi list (header WI-FI), B the Bluetooth list (BLUETOOTH), P Other devices (OTHER DEVICES); the two-line list, ≤5 rows (windowed). Rows are built by the Radxa: the switch (`Wi-Fi·Wi-Fi is on·ON`), networks (`name·Connected/Secured/Open·`), devices (`name·Connected/Not connected/New device·`), `PAIR NEW DEVICE`, `SEARCHING...`, `SEARCH AGAIN` |
...
| Notes list | `NOTES\|sel\|title·when·\|…` — sel −1 back, −2 plus, else the row in the window; ≤5 rows; no rows = NO NOTES |
| Note | `NOTE\|hdr\|line·line·…` — hdr `''` typing (the cursor follows the last line) / `B` `<` / `S` SAVE / `D` DELETE selected; the Radxa wraps the note at 30 columns (a paragraph break is an empty line) and sends the **end** that fits, first line `...` when the start is cut |
...
| Chapters | `CHAPTERS\|sel\|title·pct·HERE\|…` — sel −1 back, else the row in the window; ≤5 rows; titles ≤22 characters |
...
*   **`test_scenarios.py`** — the phone used end to end, key by key, against a whole pretend phone (scratch data folder, `SimModem`, fake Wi-Fi/Bluetooth, a real test EPUB, real tiny WAVs): texting (send, reply arriving, retry), the lock screen mark, contacts (create, duplicate refused, find, delete), calls (logged with numbers, a missed call → the person → a text with LAST CALL), reading (pages, size, chapters, resume), music (play, next, pause, background, resume), Settings (Wi-Fi password wrong then right, keyboard guard, pairing, light, mark), notes, adding from a computer, and a restart that reads everything back. Then **random keys** (16 runs × 700) from home and from inside each app must never raise, and must reach every major screen. **Every command any of it sent** must fit a frame, be drawable ASCII, draw in the emulator, and draw in the firmware's host build under ASan/UBSan (over 2,000 distinct screens).
...
### **Deploying**
*   **Radxa:** systemd `kyphone.service` runs `spi_bridge/kyphone_os.py` as root from `~/kyphone`. To update: copy `kyphone_os.py`, `simulator.py`, `home_icons.py`, `version.py` and (reader) `reader_epub.py`, `reader_layout.py`, `reader_fonts.py`, (music) `music_library.py`, `music_player.py`, (settings) `network_control.py`, (modem) `modem.py`, (adding files from a computer) `upload_server.py`; put books in `~/kyphone/data/books/` and music in `~/kyphone/data/music/` (wired playback needs no new packages: GStreamer and its Python bindings are already on the Radxa); for real texting, plug in the SIM7600G-H and set `KYPHONE_MODEM_PORT` in `kyphone.service`'s environment before restarting; `sudo systemctl restart kyphone`; check `journalctl -u kyphone` on the Radxa and `/tmp/inkplate_serial.log` **on the Mac mini** (its logger streams the Inkplate's USB serial output, one line per command it receives). The Radxa's git clone is not kept in step — files are copied in.
*   **Inkplate:** `flash_macmini.sh` (compiles, archives the image, writes only the app at `0x10000`; the bootloader and partition table do not change). **Stop `serial_log_macmini.py` first** — it holds the port — and start it again afterwards.
*   **Backups (Kyle, 2026-09-26).** Inkplate: `flash_macmini.sh` keeps **every firmware it builds** in `~/kyphone-backups/firmware/<version>.bin` and uploads only the app image at 460800 baud (921600 fails on this board's USB chip); `rollback_macmini.sh <version>` puts an earlier one back in seconds. The Inkplate stores no data of its own, so that image is a complete rollback. A full 4 MB dump (`esptool read_flash 0 0x400000`, about 6 minutes) is kept as the last resort and only redone if the bootloader or partition table changes (`FULL=1 flash_macmini.sh`). Radxa: a tarball of `spi_bridge/`, `data/` and the unit file before each deploy (seconds). A deploy takes about a minute. Python and firmware must ship **together** when a wire format changes.

### **Security / privacy constraints**
*   **Nothing typed or read reaches the log.** The service's output goes to the Radxa's journal on disk, so it logs a key as `CHAR` without its letter and a screen command by its name and length only; texts arriving are logged as `[NEW SMS]` with no sender or body (a test checks this; fixed 2026-09-25 after a review found typed Wi-Fi passwords in the journal).
...
*   `spi_bridge/upload_server.py` — the Add from a computer page (books, music, contacts over home Wi-Fi).
...
*   **e-ink refresh (0.6.7):** the Radxa decides (`_wants_full_refresh`): a **full** refresh when moving between the home screen and a feature (either way) and when the phone goes to the lock screen, **partial** everywhere else (scrolling, moving inside a feature). **The TEMPERA library forces a full refresh on every 10th `partialUpdate()` by default** (`_partialUpdateLimiter = 10`); the firmware sets `setFullUpdateThreshold(0)` in `setup()` — without it the screen flashed at random mid-scroll, and the serial log did not show it; it marks the frame with start byte 0x04. The Inkplate also does a full refresh after boot and, to clear ghosting while idle, on a lock screen redraw once 10 minutes have passed — never on a key press inside a feature. A full request on a frame replaced before it went out carries over to the newer one. Book pages still choose their own refresh in `RFOOT`.
