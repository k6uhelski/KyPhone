...
| Screen | Command |
| :--- | :--- |
| Lock | `LOCK\|time\|DAY, MON DD\|quote\|attribution\|version\|mark\|light` — the version (e.g. `0.5.0`) is drawn as "OS 0.5.0" bottom left (none sent = no label); `mark` `*` = new activity (an unread text or a missed call not yet seen on the call list), drawn right of the clock; `light` 0–8 sets the front light (the Radxa sends it with every lock screen, so a restarted Inkplate gets its light back) |
