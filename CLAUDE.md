...
| wifi | the Wi-Fi switch (Enter turns it on/off); the joined network (Enter: FORGET THIS NETWORK, on the confirmation screen); then the networks nearby, A–Z. A search runs once on opening (the switch and the joined network are shown at once, `SEARCHING...` ends the list until it answers, then one redraw); `SEARCH AGAIN` (with `No networks found` when empty) runs another. An open network connects at once; a secured one opens the password box. Esc/Q → settings |
