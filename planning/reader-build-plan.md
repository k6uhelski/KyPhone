...
**Home menu order changed (2026-09-19, Kyle, after seeing the emulator):** TEXT, CALL, READ, LISTEN, CONTACTS (texts, calls, books, music, address book), replacing the design handoff's CONTACTS-third order. Changed in `kyphone_os.HOME_MENU`, `simulator.HOME_MENU`, the firmware's `labels[]`, and `make_icons.ORDER` (icon tables regenerated); tests now reach rows by name, one test pins the order, and two new tests check that the last two rows draw the right icon when scrolled into view. Firmware and Python must be flashed/deployed together for this: an old Radxa with new firmware shows icons in the new order but Enter still follows the old one. The designer's capture (`12_home_contacts_third.png`) shows the old order and is now only used for icon bitmaps.
